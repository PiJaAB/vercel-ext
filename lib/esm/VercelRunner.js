...
import vercelPath from "./VercelBinPath.js";
...
export class NonNormalExitError extends Error {
    stdOut;
    stdErr;
    exitCode;
    signal;
    ...
    constructor(stdOut, stdErr, exitCode, signal, vercelVersion, interactive) {
        ...
        if (stdErr && !interactive) {
            const newMessage = stripAnsi(stdErr).replace(/^Error:\s*/, "");
            if (newMessage) {
                message = newMessage;
            }
        }
 ...
 */
export class VercelBaseError extends Error {
    stdOut;
    stdErr;
    vercelVersion;
...
export class VercelError extends VercelBaseError {
    code;
    vercelVersion;
...
export class VercelMultiError extends VercelBaseError {
    codes;
    /**
     * @param {string} stderr
...
export const INTERACTIVE = Symbol("interactive");
export const YES = Symbol("yes");
export const NONINTERACTIVE = Symbol("noninteractive");
export class Vercel {
    token;
    teamSlug;
    teamId;
    ...
    async runVercel(mode, ...args) {
        ...
        else if (mode != null && mode !== NONINTERACTIVE) {
            throw new Error(`Invalid mode ${mode}`);
        }
        return new Promise((resolve, reject) => {
            const command = vercelPath;
            const allArgs = [
                ...this.preArgs,
                ...args
    ...
    async login({ mode } = {}) {
        if (mode !== INTERACTIVE && mode != null) {
            return Promise.reject(new Error("Login must be interactive"));
        }
        try {
            return await this.runVercel(INTERACTIVE, "login");
        }
        catch (err) {
    ...
    async isLinked() {
        try {
            const hasLink = await this.runVercel(NONINTERACTIVE, "env", "ls").then(() => {
                return true;
            }, (err) => {
                if (err instanceof NonNormalExitError &&
